...
TARGET_SKILLS = {
    ...
    "fine-tuning", "lora", "qlora", "peft", "fine-tuning llms",
    # Core
    "python",
    # Evaluation
...
def calculate_score(cand):
    ...
    if not is_disqualified:
        # Check title terms
        if any(term in current_title for term in ["ai engineer", "ml engineer", "machine learning engineer", "nlp engineer", "retrieval engineer"]):
            title_score = 25.0
        elif any(term in current_title for term in ["data scientist", "software engineer", "lead engineer"]):
            title_score = 18.0
        else:
            title_score = 5.0
    else:
    ...
    # Max out skill score at 30.0
    skill_score = min(skill_points * 4.0, 30.0)
    ...
    if len(employers) > 0:
        if num_consulting == len(employers):
            # Worked ONLY in service/consulting companies
            company_score = 0.0
        else:
            ...
            if num_product > 0:
                company_score += 5.0
            # Extra points for top firms or modern startups
    ...
    # 5. Behavioral Multipliers (multiplier ranges from ~0.5 to 1.35)
    multiplier = 1.0
    ...
    if "noida" in loc or "pune" in loc or "delhi" in loc or "ncr" in loc:
        ...
        # Outside India, likely requires work visa sponsorship (not provided)
        multiplier *= 0.4
    ...
    # Clean output score range (0.0 to 1.0) for submission rules
    final_score = max(0.001, min(final_score, 100.0)) / 100.0
    
    return final_score, skills_found

def generate_reasoning(cand, rank, score_val, matched_skills):
    profile = cand.get("profile", {})
    ...
    if not skills_subset:
        skills_subset = ["Python", "Applied ML"]
        
    skills_text = ", ".join(skills_subset)
    
    # We create distinct templates depending on the rank range
    if rank <= 10:
        return f"Elite candidate with {exp} years of experience; built scalable vector databases and fine-tuned models at {emp_str}. Highly active platform signals, {notice}-day notice period, and Noida/Pune location readiness make them a perfect founding team fit."
    elif rank <= 30:
        return f"Exceptional background matching 'product over research' with {exp} years of experience. Demonstrated depth in {skills_text} at {emp_str}; strong recent activity and short notice period align directly with our JD."
    elif rank <= 70:
        ...
        return f"Capable engineer with {exp} years of experience. Practical background in Python and {skills_text}; good platform engagement signals but notices or location represent minor trade-offs."

def rank_candidates(candidates_path, output_path):
    print(f"Loading candidates from {candidates_path}...")
    candidates = []
    
    # Handle both gzipped and plain jsonl files
    ...
    for cand in candidates:
        ...
        if score < 0:
            honeypot_count += 1
            # Do not include honeypots in scored candidates
            continue
    ...
    with open(output_path, "w", encoding="utf-8", newline="") as f:
        ...
        for i, (score, cand, matched_skills) in enumerate(top_100):
            ...
            reasoning = generate_reasoning(cand, rank, score, matched_skills)
            writer.writerow([cid, rank, round(score, 4), reasoning])
            
    print("Ranking complete. CSV file successfully written.")

def main():
    parser = argparse.ArgumentParser(description="Rank candidates for Redrob AI Senior AI Engineer challenge.")
